...
char* get_email()
{
    ...
    gets(email);
    int i = 0, at_found = 0, dot_found = 0, at_index = 0, dot_index = 0;
    while(1)
    {
        if(strlen(email) < 5)
        {
        ...
        if(at_index >= dot_index || abs(at_index - dot_index) == 1 || at_index == 0 || dot_index == (strlen(email) - 1))
        {
            printf("Invalid email\nEnter another E-mail: ");
            at_found = 0;
            at_index = 0;
...
void PrintSorted(Account accounts[],int n)
{
    ...
    for(k; k < arr_len; k++)
    {
        strcpy(arr[k].accountnumber, accounts[k].accountnumber);
        strcpy(arr[k].name, accounts[k].name);
        strcpy(arr[k].email, accounts[k].email);
        arr[k].balance = accounts[k].balance;
        strcpy(arr[k].mobile, accounts[k].mobile);
        arr[k].dateopened.month = accounts[k].dateopened.month;
        arr[k].dateopened.year = accounts[k].dateopened.year;
    }
    ...
    case 1:
        ...
        break;
    case 2:
        sortby_BALANCE(arr,n);
        break;
    case 3:
        sortby_DATEOPENED(arr,n);
        break;
    default:
